...
# python3 tools/trainer.py --mode train --freeze --checkpoint models/CovidAID_transfered.pth.tar --bs 16 --save models/first_try/
...
class Trainer:
    ...
    def evaluate(self, TEST_IMAGE_LIST, BATCH_SIZE=64, cm_path='cm', roc_path='roc'):
        ...
        """
        print('Runnning predictions')
        gt, pred = self.predict(TEST_IMAGE_LIST, BATCH_SIZE=16)
        print('here')
        print (pred)

        # Compute ROC scores
        # labels = ['Normal', 'Bacterial', 'Viral', 'COVID-19']
        labels = ['not_in_icu', 'in_icu']
